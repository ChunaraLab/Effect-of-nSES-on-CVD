...
mesa_bla_mediation <- do.call(rbind, lapply(files, read_csv_add_filename)) %>%
...
mesa_bla_mediation$Effect <- factor(mesa_bla_mediation$Effect, levels =unique(mesa_bla_mediation$Effect))
...
plot <- ggplot(plt_dat, aes(x = Estimate, y = ind_mediator, color = Effect)) +
            ...
            theme_minimal() +
            ggtitle("nSES as exposure using mesa_bla_pre data") +
...
plot <- ggplot(plt_dat, aes(x = Mean, y = ind_moderator, color = Data)) +
            ...
            geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
            theme_minimal() +
            ggtitle("Moderation of individual features on nSES in mesa_bla") +
            theme(axis.text = element_text(size = 15),
                  axis.title = element_text(size = 15),
                  legend.text = element_text(size = 15),
                  legend.title = element_text(size = 15),
