...
```{r}
dosage <- data.frame(nRS = -2.5:1)  # based on the exposure range; can be different for mesa and jhs
response <- predict(modelA, newdata = dosage, type = "response")
plt <- ggplot(data = data.frame(nRS = -2.5:1, Probability = response), aes(x = nRS, y = Probability)) +
  geom_line() +
