...
data <- data[propensity_density <= threshold, ]    
propensity_density <- propensity_density[propensity_density <= threshold]
data$propensity_scores <- 1/propensity_density
data$weights <- propensity_density


# Plot the distribution of propensity scores
ggplot(data, aes(x = propensity_scores)) +
...
balance_before <- svymean(~nPhysFac+nFavFood+nRS+FamIncome+nutrition+PhysAct+currentSmoker+alc+age+Diabetes+hdl+totchol+sbp+race+site, 
                          design = svydesign(ids = ~1, data = data))

# Check balance after weighting
balance_after <- svymean(~nPhysFac+nFavFood+nRS+FamIncome+nutrition+PhysAct+currentSmoker+alc+age+Diabetes+hdl+totchol+sbp+race+site, 
                         design = design)

# Compare balance before and after weighting
balance_check <- bind_rows(
  tibble(
    covariate = names(balance_before),
    mean_before = as.numeric(balance_before),
    mean_after = as.numeric(balance_after)
  )
)
...
ps_model <- glm(nSES ~ nPhysFac + nFavFood + nRS + FamIncome + nutrition + PhysAct +
                  currentSmoker + alc + age + Diabetes + hdl + totchol + sbp + race + site, 
                data = data, family = gaussian)

# Predicted values (propensity scores)
data$ps <- predict(ps_model, type = "response")

# Create weights
data$weights <- 1 / data$ps

# Check balance before weighting
bal.tab(nSES ~ nPhysFac + nFavFood + nRS + FamIncome + nutrition + PhysAct +
          currentSmoker + alc + age + Diabetes + hdl + totchol + sbp + race + site, 
        data = data, un = TRUE, continuous = "std")

# Apply weights using the survey package
design <- svydesign(ids = ~1, weights = ~weights, data = data)

# Check balance after weighting
bal.tab(nSES ~ nPhysFac + nFavFood + nRS + FamIncome + nutrition + PhysAct +
          currentSmoker + alc + age + Diabetes + hdl + totchol + sbp + race + site, 
        data = data, weights = data$weights, method = "weighting", continuous = "std")

# Output balance tables
print(bal.tab(nSES ~ nPhysFac + nFavFood + nRS + FamIncome + nutrition + PhysAct +
                currentSmoker + alc + age + Diabetes + hdl + totchol + sbp + race + site, 
              data = data, un = TRUE, continuous = "std"))
