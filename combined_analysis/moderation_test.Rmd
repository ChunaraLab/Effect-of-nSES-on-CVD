...
for (combo in all_combos) {
  ...
  last_term_name <- tail(names(coef(interact_mod)), 1)
  last_term_ci <- confint(interact_mod, parm = last_term_name)
  interaction_ci_lwr <- last_term_ci["2.5 %"]
  interaction_ci_upr <- last_term_ci["97.5 %"]
...
```{r}
write.csv(report_df, "../results/moderation_result/moderation_result_0529.csv", row.names = FALSE, quote = TRUE, sep = ",", fileEncoding = "UTF-8")
```
...
write.csv(moderation_significant, "../results/moderation_result/moderation_significant_0529.csv", row.names = FALSE, quote = TRUE, sep = ",", fileEncoding = "UTF-8")
```